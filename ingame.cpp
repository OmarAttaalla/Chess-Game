...
void InGame::gameState()
{
    if(cb.whiteKing->checkmate(cb))
    {
        stop = true;
...
QLabel *InGame::rookSearch(const QPushButton *coordinate)
{
    ...
    } else
    {
        ...
        if(piece != 0) { return piece; }
        piece = pieceTypeMatch(coordinate, ui->wRook2);
        if(piece != 0) { return piece; }
    }
...
QLabel *InGame::queenSearch(const QPushButton *coordinate)
{
    ...
    if(turn)
    {
        piece = pieceTypeMatch(coordinate, ui->bQueen);
        if(piece != 0) { return piece; }

    } else
    {
        piece = pieceTypeMatch(coordinate, ui->wQueen);
        if(piece != 0) { return piece; }
    }
...
QLabel *InGame::kingSearch(const QPushButton *coordinate)
{
    ...
    if(turn)
    {
        piece = pieceTypeMatch(coordinate, ui->bKing);
        if(piece != 0) { return piece; }

    } else
    {
        piece = pieceTypeMatch(coordinate, ui->wKing);
        if(piece != 0) { return piece; }
    }
...
void InGame::pieceCaptured(QLabel *piece)
{
    if(!piece) { return; }

    //piece->clear();
    piece->setGeometry(QRect(1000, 1000, 50, 50));
}
...
void InGame::move(const QPushButton *coordinate)
{
    if(moving)
    {
        ...
        for(Square* sq: s)
        {
            ...
            {
                Square* prev = cp->get_placeAt();
                ChessPiece* prevPiece = sq->getPiece();
                cp->moves(sq, cb);

                if(turn)
                {
                    if(cb.blackKing->checkScan(cb))
                    {
                        sq->setPiece(prevPiece);
                        cb.addPiece(prevPiece);
                        prev->setPiece(cp);
                        cp->set_placeAt(prev);
                        break;
                    }
...
void InGame::on_clicked(const QPushButton *coordinate)
{
    if(!stop)
    {
        ...
        {
            move(coordinate);
            pieceCaptured(pieceTypeSearch(coordinate));
